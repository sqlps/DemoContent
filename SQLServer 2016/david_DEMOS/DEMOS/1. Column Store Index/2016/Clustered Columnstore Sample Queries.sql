...
ALTER DATABASE AdventureworksDW2016CTP3 SET compatibility_level = 120
GO
-- you will see that this query now runs in rowmode 
SELECT ProductKey	,OrderDateKey
	,LEAD(OrderQuantity, 1, 0) OVER (ORDER BY OrderDateKey) AS NextQuota
FROM FactResellerSalesXL_CCI
WHERE orderdatekey IN (	20060301,20060601)
...
-- Look at the Rows coming out of the SCAN, invariably in the past, 
-- 11 million rows would flow out and be filtered to 1 row
...
-- Also notice no segments were eliminated. 
-- There is an optimization where you see a PROBE BITMAP filter pushed to the SCAN

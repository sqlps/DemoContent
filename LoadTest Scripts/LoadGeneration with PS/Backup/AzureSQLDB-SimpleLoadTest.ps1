...
$RunTimeInMins = 2
$SleepyTimeSecs = 5
$Loops = ($RunTimeInMins * 60)/ $SleepyTimeSecs
$PBIModulePath = 'C:\Demos\Azure\LoadGeneration\PowerBI'
$BaselineServer = 'zbqfsz8os9.database.windows.net'
...
$Counter = 1
Invoke-Sqlcmd -ServerInstance $BaselineServer  -Database $BaseLineDB -Username $Login -Password $pwd -Query "exec LoadTest.usp_ResetTables"
Invoke-Sqlcmd -ServerInstance $CompareServer -Database $CompareDB -Username $Login -Password $pwd -Query "exec LoadTest.usp_ResetTables"

#Kickoff Workload
$program = "C:\Program Files\Microsoft Corporation\RMLUtils\ostress.exe"
$arguments = "-Q""LoadTest.usp_ReadWorkLoad 'ReadWorkload-Basic'"" -S""$BaselineServer"" -d$BaselineDB -U$Login -P$pwd -t10 -o""ReadWorkload-Basic"" -q -n$BaselineThreads -r$BaselineRuns"
Start-Process -FilePath "$program" -ArgumentList "$arguments"

$program = "C:\Program Files\Microsoft Corporation\RMLUtils\ostress.exe"
$arguments = "-Q""LoadTest.usp_ReadWorkLoad 'ReadWorkload-Premium'"" -S""$CompareServer"" -d$CompareDB -U$Login -P$pwd -t10 -o""ReadWorkload-Premium"" -q -n$CompareThreads -r$CompareRuns"
Start-Process -FilePath "$program" -ArgumentList "$arguments"


#Open PowerBi Report
$ie = New-Object -ComObject InternetExplorer.Application
$ie.Navigate("https://msit.powerbi.com/dashboards/9c231499-3761-4464-bb8c-c8f1bf236dd2")
$ie.Visible = $true
#>
Do 
{
    # Read Load Test Data
    $Results1 = Invoke-Sqlcmd -ServerInstance $BaselineServer -Database $BaselineDB -Username $Login -Password $pwd -Query "exec LoadTest.usp_GetPerfMetrics 'ReadWorkload-Basic'"
    $Results2 = Invoke-Sqlcmd -ServerInstance $CompareServer  -Database $CompareDB -Username $Login -Password $pwd -Query "exec LoadTest.usp_GetPerfMetrics 'ReadWorkload-Premium'"
